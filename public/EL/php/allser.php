<?php header('Access-Control-Allow-Origin: *');
	...
	//	$objConnect = mysql_connect("localhost","whp0808153026AP7","qh7LaWHT8q") or die("Error Connect to Database");
     //   $objDB = mysql_select_db("rolex");
        $lim = $_POST["lim"];
        $strSQL = "SELECT * FROM product where  status =1 AND (name_ LIKE '%$lim%' OR des_ LIKE '%$lim%' OR paped LIKE '%$lim%' )  ORDER BY ID DESC";        
       ...
       $result = $conn->query($strSQL);
        ...
        // {
            ...
            array_push($resultMM,$objResult["mm"]);
